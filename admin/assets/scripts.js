...
jQuery(function($){
    ...
    /* определяем показ блока опций при загрузке страницы*/
    ...
    /*if(rcl_url_params['page'] && rcl_url_params['page'] == 'rcl-options'){
        var viewBox = false;
        if(viewBox = rcl_url_params['options']){
            $('#'+viewBox+'-options-box').addClass('active');
        }else{
            $('#primary-options-box').addClass('active');
        }
    }*/
    /***/
    ...
    $(".rcl-parent-field").find("input, select").each(function(){
        var parent_id = $(this).attr('id'); var parent_val = $(this).val();
...
function rcl_onclick_options_label(e){
    ...
    if(jQuery('#'+viewBox+'-options-box').hasClass('active')) return false;
