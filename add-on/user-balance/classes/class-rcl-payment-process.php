...
class Rcl_Payment_Process extends Rcl_Payment_Core{
    ...
    function get_id_is_payment(){
        ...
        foreach(Rcl_Gateways()->gateways as $id => $className){
            ...
            if(isset($_REQUEST[Rcl_Gateways()->gateway($id)->request])) return $id;
    ...
    function payment_process($gateway_id){
        ...
        switch($this->post_id){
            case $this->page_result: Rcl_Gateways()->gateway($gateway_id)->result($this); break;
            case $this->page_success: Rcl_Gateways()->gateway($gateway_id)->success($this); break;
        }
