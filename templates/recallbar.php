...
<div id="recallbar">
	...
	<?php if(is_user_logged_in() && current_user_can('activate_plugins')){ // если залогинен и есть полномочия (тот, кто может активировать плагины - явно большой босс)
			$r_lnk .= '<li><a href="/"><i class="fa fa-home"></i><span>' . __('Home') . '</span></a></li>'
					. '<li><a href="' . admin_url() . '"><i class="fa fa-external-link-square"></i><span>' . __('Dashboard') . '</span></a></li>'
					. '<li><a href="' . $rcl_user_URL . '"><i class="fa fa-user"></i><span>' . __('Personal cabinet','rcl') . '</span></a></li>'
		...
		if($rcl_options['login_form_recall']==1){ // Каждая загружается отдельно
			...
			$r_lnk .= '<li><a href="/"><i class="fa fa-home"></i><span>' . __('Home') . '</span></a></li>'
					...
					. '<li><a href="' . $page_in_out . 'action-rcl=login"><i class="fa fa-sign-in"></i><span>' . __('Login','rcl') . '</span></a></li>';
			echo $r_lnk;
		}
			...
			echo '<li><a href="' . esc_url(wp_login_url('/')) . '"><i class="fa fa-sign-in"></i><span>' . __('Login','rcl') . '</span></a></li>';
		}
			...
			$r_lnk .= '<li><a href="/"><i class="fa fa-home"></i><span>' . __('Home') . '</span></a></li>'
					...
					. '<li><a href="#" class="rcl-login"><i class="fa fa-sign-in"></i><span>' . __('Login','rcl') . '</span></a></li>';
			echo $r_lnk;
		}
	} ?>
	</ul>
